# Implementation Plan - Session Management and JTI Integration
...
## Phase 2: Login Flow (JTI Generation & Persistence)
Goal: Update the Login process to generate a UUID JTI and save session metadata.
...
- [ ] Task: Integrate `sessionService.Create` in the Login flow.
    - [ ] Write failing test verifying `sessionService.Create` is called with JTI and user metadata.
    - [ ] Implement the call to `sessionService.Create` before returning the tokens.
    - [ ] Ensure TTL matches the Refresh Token expiry.
- [ ] Task: Conductor - User Manual Verification 'Phase 2: Login Flow (JTI Generation & Persistence)' (Protocol in workflow.md)
...
- [ ] Task: Update `AuthUseCase.Validate` to extract and verify JTI.
    ...
    - [ ] Call `sessionService.GetByID` (using JTI) to verify session status in DynamoDB.
    - [ ] Ensure the validation fails if the session is not found or is marked as invalid.
...
- [ ] Task: Perform final end-to-end manual test in LocalStack.
    - [ ] Verify DynamoDB records after a successful login.
